...
from app.services.dhcp.utils import (
...
class DHCPResponseFactory:
    ...
    def build(cls, dhcp_type: int, your_ip: str, request_packet: Packet) -> Packet:
        ...
        _config: DHCPConfig | None = cls._config
        ...
        _bootp_opts = BOOTPOptions(
            ...
            yiaddr=your_ip,
            siaddr=_config.server_ip,
            giaddr="0.0.0.0",
            flags=_config.flags,
        )

        dst_mac = (
            _config.broadcast_mac
            if request_packet[BOOTP].flags & 0x8000
            else request_packet[Ether].src
        )
        dst_ip = (
            _config.broadcast_ip
            if request_packet[BOOTP].flags & 0x8000
            else request_packet[BOOTP].ciaddr or your_ip
        )

        return (
            Ether(src=_config.server_mac, dst=dst_mac)
            / IP(src=_config.server_ip, dst=dst_ip)
            / UDP(sport=_config.port, dport=request_packet[UDP].sport)
...
class DHCPMessage:
    def __init__(self, packet: Packet):
        ...
        self.vendor_class_id: str = extract_vendor_class_id(packet)
