...
from app.services.neural_net.models import DomainClassifier, TrainingProgress
from app.services.neural_net.training_models import ModelPredictor, ModelTrainer
...
BLACKLISTS_FILE = Path(ROOT_PATH / config.get("dns").get("blacklists_config").get("path"))
WHITELISTS_FILE = Path(ROOT_PATH / config.get("dns").get("whitelists_config").get("path"))
...
class NNDomainClassifierService:
    ...
    def _get_whitelists(cls) -> set:
        return set(get_local_file(file_path=WHITELISTS_FILE).get("payload", {}).get("urls", []))

    @classmethod
    def _get_blacklists(cls) -> set:
        _adserver_list = set(get_latest_adservers()) if USE_ADSERVER_LIST else set()
        return (
            set(get_local_file(file_path=BLACKLISTS_FILE).get("payload", {}).get("urls", []))
            | _adserver_list
        )
    ...
    @staticmethod
    def _get_optimizer(
        model, learning_rate: float = LEARNING_RATE, weigh_decay: float = WEIGHT_DECAY
    ) -> AdamW:
        return AdamW(params=model.parameters(), lr=learning_rate, weight_decay=weigh_decay)
    ...
    @classmethod
    def train_new_model(cls) -> Generator[TrainingProgress, None, None]:
        """
        ...
        with cls._lock:
            ...
            try:
                ...
                _model_accuracy = _trainer.eval(
                    test_dataset=generate_testing_dataset(), model=_trained_model
                )
