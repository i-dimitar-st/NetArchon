...
from app.services.neural_net.model_utils import (
    evaluate_model,
...
class ModelPredictor:
    ...
    def predict(self, model: DomainClassifier, domains: list[str]) -> list[dict[str, float]]:
        ...
        results = []
        for domain, probability in zip(domains, _predictions):
            results.append({"domain": str(domain), "probability": float(probability)})
        del _predictions
        return sorted(results, key=lambda each: each["probability"], reverse=True)
