    ...
    useEffect(() => {
        const fetchLeases = async () => {
            ...
            try {
                ...
                if (!jsonRes?.payload?.leases) throw new Error(jsonRes.error || 'Unknown error');
                setLeases(jsonRes.payload.leases || []);
                setTableHeaderNames(['type', 'ip', 'mac', 'hostname', 'timestamp', 'expiry_time']);
                console.info('DHCP Leases Fetched');
            } catch (err) {
                console.error(err);
            } finally {
                setLoading(false);
    ...
    useEffect(() => {
        const fetchStats = async () => {
            ...
            try {
                ...
                if (!res.ok) throw new Error('Server error');
                if (!jsonRes?.payload?.statistics) throw new Error(jsonRes.error || 'Unknown error');

                setStats(jsonRes.payload.statistics);
                setMaxValue(jsonRes.payload.statistics ?? 1);
                console.info('DHCP Stats Fetched');
            } catch (err) {
                console.error(err);
            } finally {
                setLoading(false);
