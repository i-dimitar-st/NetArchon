...
function ActionsTab({ token, dnsHistory, predictions, setPredictions, setBlacklists, setWhitelists }) {
    ...
    const handleTrainModel = async () => {
        ...
        try {
            ...
            for await (const chunk of fetcherStreaming({ token, body: reqBody })) {
                ...
                const status = payload.status || chunk.status || 'Training';
    ...
    const handlePredict = async () => {
        ...
        try {
            ...
            if (!json?.payload?.predictions) throw new Error('Payload missing');
    ...
    useEffect(() => {
        const fetchBlacklists = async () => {
            ...
            try {
                ...
                if (!json?.payload?.blacklist) throw new Error('Payload missing');

                setBlacklists(json.payload.blacklist);
                console.info('Blacklist fetched');
            } catch (err) {
                console.error(err);
            } finally {
                setLoading(false);
    ...
    useEffect(() => {
        const fetchWhitelists = async () => {
            ...
            try {
                ...
                if (!json?.payload?.whitelist) throw new Error('Payload missing');

                setWhitelists(json.payload.whitelist);
                console.info('Whitelist fetched');
            } catch (err) {
                console.error(err);
            } finally {
                setLoading(false);
    ...
    useEffect(() => {
        const fetchDnsHistory = async () => {
            ...
            try {
                ...
                const data = json.payload.history;
