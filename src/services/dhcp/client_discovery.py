...
from src.services.dhcp.live_clients import LiveClients
from src.services.dhcp.models import (
    ClientDiscoveryConfig,
    DHCPArpClient,
)
...
def _get_network(
    ip: IPv4Address,
    cidr: int,
    min_cidr: int = MIN_CIDR,
    max_cidr: int = MAX_CIDR,
) -> IPv4Network:
    ...
    def init(cls, logger):
        with cls._lock:
            ...
            cls.live_clients = LiveClients(
                max_ctr=cls.config.max_counter,
                min_ctr=cls.config.min_counter,
            )
