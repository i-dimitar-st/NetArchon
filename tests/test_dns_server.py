...
def print_response(response:DNSRecord = None):
    ...
    print(f"id:{response.header.id} | rcode:{response.header.rcode} qname:{response.q.qname} qtype:{response.q.qtype}")
    print(f"answers:{len(response.rr)} | auth:{len(response.auth)} | addl:{len(response.ar)} | ip_ttl:{ips_ttls}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(_valid_domains)) as executor:
            ...
            for future in concurrent.futures.as_completed(futures, timeout=15):
                ...
                try:
                    ...
                    self.assertLess(delay, TIMEOUT + 1, msg=f"Slow response from {domain}: {delay:.2f}s")
